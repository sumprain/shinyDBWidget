#' @export
widgetNumericDB <- function(inputId, label, value = NULL, isRequired = FALSE, readOnly = FALSE, onlyDataEntry = FALSE, defaultVal = NULL, decimal = NULL) {
  ...
  tagList(singleton(tags$head(
    ...
    tags$script(src="js/widgetNumericDB.js")
  )),
  tags$body(tags$div(id = inputId, class = "widgetNumericDB widgetDB",
                     tags$p(tags$b(label)),
                     tags$div(class = "divNoEdit", `data-isReqd` = isRequired, `data-readOnly` = readOnly, `data-onlyDataEntry` = dataEntry, `data-decimal` = decimal, p(value)),
...
#' @export
updateWidgetNumericDB <- function(session, inputId, isUpdatedInDB = FALSE, value = NULL) {
  ...
  if (!is.null(value) && !is.numeric(value)) {
    msg <- "Number required on update"
    value <- ""
  } else msg <- ""

  session$sendCustomMessage(type = "sendToWidgetNumericDB", jsonlite::toJSON(list(id = inputId, isUpdatedInDB = isUpdatedInDB, value = value, msg = msg)))

}

#' @export
validateWidgetNumericDB <- function(session, input, inputId, condition, message) {
  ...
  if (isValidated == FALSE) {
    session$sendCustomMessage(type = "validateWidgetNumericDB", jsonlite::toJSON(list(id = inputId, message = message)))
    #input[[inputId]] <- NULL
  }
...
#' @export
registerWidgetNumericDB <- function(fun = function(data, ...) return(data)) {
  environment(fun) <- parent.frame()
  shiny::registerInputHandler(type = "shinyDBWidget.widgetNumericDBBinding", fun, force = TRUE)
}
