#' @export
widgetTBDB <- function(inputId, label, value, isRequired = FALSE, readOnly = FALSE, rows = 8, cols = 33, onlyDataEntry = FALSE) {
  ...
  tagList(singleton(tags$head(
    ...
    tags$script(src="js/widgetTBDB.js")
  )),
  tags$body(tags$div(id = inputId, class = "widgetTBDB widgetDB",
                     tags$p(tags$b(label)),
                     ...
                     tags$div(class = "divEdit",
                              tags$textarea(class = "tbEdit", rows = rows, cols = cols),
...
#' @export
updateWidgetTBDB <- function(session, inputId, isUpdatedInDB = FALSE, value = NULL) {
  ...
  session$sendCustomMessage(type = "sendToWidgetTBDB", jsonlite::toJSON(list(id = inputId, isUpdatedInDB = isUpdatedInDB, value = value), null = 'null'))

}

#' @export
validateWidgetTBDB <- function(session, input, inputId, condition, message) {
  ...
  if (isValidated == FALSE) {
    session$sendCustomMessage(type = "validateWidgetTBDB", jsonlite::toJSON(list(id = inputId, message = message)))
    #input[[inputId]] <- NULL
  }
...
#' @export
registerWidgetTBDB <- function(fun = function(data, ...) return(data)) {
  environment(fun) <- parent.frame()
  shiny::registerInputHandler(type = "shinyDBWidget.widgetTBDBBinding", fun, force = TRUE)
}
